...
export const orderRouter = {
    ...
    .handler(async ({ input }) => {
      ...
      const orders = sessions.data.map((session) => {
        ...
        return {
          ...
          shippingAddress: shipping?.address ? formatAddress(shipping.address) : null,
  ...
  get: protectedProcedure.input(z.object({ id: z.string().min(1) })).handler(async ({ input }) => {
  ...
  updateShipping: protectedProcedure
    ...
    .handler(async ({ input }) => {
      ...
      const shippedAt =
        input.shippingStatus === 'shipped' && !existing?.shippedAt
          ? now
          : (existing?.shippedAt ?? null);
      const deliveredAt =
        input.shippingStatus === 'delivered' && !existing?.deliveredAt
          ? now
      ...
      if (existing) {
        await db
          ...
          .set({
            ...
            trackingNumber:
              input.trackingNumber !== undefined ? input.trackingNumber : existing.trackingNumber,
